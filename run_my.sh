...
cur=$PWD
cd CIFAR-100
python config_distillation.py --teacher_path "$cur"/experiments/models/"$teacher"_"$method"_trial_"$trial"/"$teacher"_last.pth --model_s "$student" --gpu "$gpu" --beta "$beta" --alpha "$alpha" --gamma "$gamma"
